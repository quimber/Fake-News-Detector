...
def function(s):
    ...
    j=0
    list=[]
    for url in search(s+"BBC", lang='es', stop=5):
        list.append(url)
        j=j+1
    return list[0]
...
def function_1(s):
    ...
    j=0
    list=[]
    for url in search(s+"CNN", lang='es', stop=5):
        list.append(url)
        j=j+1
    return list[0]
...
'''def function_2(s):
    ...
    j=0
    list=[]
    for url in search(s+"Fox news", lang='es', stop=5):
        list.append(url)
        j=j+1
    return list[0]'''
...
article_1 = NewsPlease.from_url(function_1(paraalpha))
#article_2 = NewsPlease.from_url(function_2(paraalpha))

parabeta=article.text
parabeta_1=article_1.text
#parabeta_2=article_2.text
y=paragraph_sim(paraalpha,parabeta)
y_1=paragraph_sim(paraalpha,parabeta_1)
#y_2=paragraph_sim(paraalpha,parabeta_2)
delta=(y+y_1)/2
print delta
